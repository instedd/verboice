-module(call_log).
-export([error/3, info/3, trace/3, cancel_active_calls_started_before/1, calls_started_before/1]).
...
calls_started_before(StartedAt) ->
  call_log:find_all([{state, "active"}, {started_at, '<', StartedAt}]).
