...
class CallFlowsController < ApplicationController
  ...
  def show
    respond_to do |format|
      ...
      format.vrb do
        render :text => @call_flow.user_flow.to_yaml
      end
    end
  end
  ...
  def create
    #toDo: check if this can be changed in persisted_variables_controller too.
    @project = current_account.projects.includes(:call_flows).find(params[:project_id])
  ...
  def import_call_flow
    if params[:vrb].blank?
      ...
      begin
        ...
        redirect_to({:action => :edit}, {:notice => 'Invalid file'})
      end
    end
  end
