...
class PersistedVariablesController < ApplicationController
  ...
  before_filter :load_persisted_variable, only: [:show, :edit, :update, :destroy]
  ...
  def new
    ...
    @contact = @persisted_variable.contact = current_account.contacts.find(params[:contact_id])
  end
  ...
  def create
    ...
    if @persisted_variable.save
      redirect_to @persisted_variable, notice: 'Persisted variable was successfully created.'
    else
      render action: "new"
    end
  end
  ...
  def update
    if @persisted_variable.update_attributes(params[:persisted_variable])
      redirect_to @persisted_variable, notice: 'Persisted variable was successfully updated.'
    else
      render action: "edit"
    end
  end
  ...
  def load_persisted_variable
    @persisted_variable = current_account.persisted_variables.includes(:contact).find(params[:id])
