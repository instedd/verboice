...
class FreeswitchAdapterTest < ActiveSupport::TestCase
  ...
  context "capture" do
    should "capture one digit" do
      @context.expects(:read).with('silence_stream://1', :min => 1, :max => 1, :terminators => '', :timeout => 5000, :variable => 'capture').returns('1')
      value = @adapter.capture :min => 1, :max => 1, :finish_on_key => '', :timeout => 5
      assert_equal '1', value
    end

    should "capture non default options" do
      @context.expects(:read).with('silence_stream://1', :min => 2, :max => 5, :terminators => '*', :timeout => 3000, :variable => 'capture').returns('1')
      value = @adapter.capture :min => 2, :max => 5, :finish_on_key => '*', :timeout => 3
      assert_equal '1', value
    end

    should "capture while playing" do
      @context.expects(:read).with('some_file', :min => 2, :max => 5, :terminators => '*', :timeout => 3000, :variable => 'capture').returns('1')
