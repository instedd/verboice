...
describe ScheduledCall do
  ...
  it { is_expected.to validate_presence_of(:call_flow) }
  it { is_expected.to validate_presence_of(:channel) }
  it { is_expected.to validate_presence_of(:time_zone) }
  it { is_expected.to validate_presence_of(:from_time) }
  it { is_expected.to validate_presence_of(:to_time) }
  ...
  it "should find matched contacts" do
    ...
    expect(ContactsFinder).to receive(:for).with(scheduled_call.project).and_return(finder)
    expect(finder).to receive(:find).with(scheduled_call.filters).and_return([1,2,3])

    expect(scheduled_call.matched_contacts).to eq([1,2,3])
  end

  it 'should delete related jobs' do
    expect(Delayed::Job.where(scheduled_call_id: scheduled_call.id).count).to eq(1)
    scheduled_call.delete_jobs
    expect(Delayed::Job.where(scheduled_call_id: scheduled_call.id).count).to eq(0)
  end
  ...
  describe 'calls' do
    before :each do
      ...
      allow(scheduled_call).to receive(:matched_contacts).and_return([@contact_a, @contact_b])
    end
    ...
    it "should make calls" do
      expect(scheduled_call.channel).to receive(:call).with(@contact_a.first_address, expected_options.merge(contact_id: @contact_a.id))
      expect(scheduled_call.channel).to receive(:call).with(@contact_b.first_address, expected_options.merge(contact_id: @contact_b.id))
    ...
    it "should not make calls if disabled" do
      ...
      expect(scheduled_call.channel).not_to receive(:call)
    ...
    describe 'order' do
      before :each do
        ...
        allow(scheduled_call).to receive(:matched_contacts).and_return([@contact_a, @contact_b, @contact_c])
      end
      ...
      it 'should call contacts by last called order' do
        ...
        expect(scheduled_call.channel).to receive(:call).with(@contact_b.first_address, anything).ordered
        expect(scheduled_call.channel).to receive(:call).with(@contact_c.first_address, anything).ordered
        expect(scheduled_call.channel).to receive(:call).with(@contact_a.first_address, anything).ordered
  ...
  describe 'triggers' do
    ...
    it 'should delete existing jobs when updated' do
      expect(scheduled_call).to receive(:delete_jobs)
      scheduled_call.name = "new #{scheduled_call.name}"
      scheduled_call.save
    end
  ...
  describe 'next run at' do
    ...
    it 'should return next occurrence from now' do
      ...
      expect(scheduled_call.next_run_at).to eq(expected)
    end
    ...
    it 'should not return a time in the past' do
      ...
      expect(scheduled_call.next_run_at).to be > now
    end
  end
  ...
  describe 'schedule job' do
    ...
    before(:each) do
      allow(scheduled_call).to receive(:next_run_at).and_return(next_run_at_time)
    end
    ...
    it 'should schedule job for next occurrence' do
      ...
      job = double('job')
      expect(Jobs::ScheduledCallJob).to receive(:new)
